...
//		System.out.println("잘 실행되나?");
		/*
		 * GET 방식으로 요청했기 때문에 doGet 메서드가 호출됨
		 * 
		 * 첫 번째 매개 변수인 HttpServletRequest request에는 요청 시 전달된 내용들이 담김(사용자가 입력한 값, 전송 방식, 요청한 사용자의 ip 등)
		 * 
		 * 두 번째 매개 변수인 HttpServletResponse response에는 요청 처리 후 응답할 때 사용하는 객체
		 * 
		 * 요청을 처리하기 위해 요청 시 전달된 값(사용자가 입력한 값)들을 뽑는다.
		 * request의 parameter 영역 안에 존재 => key-value 세트로 담겨있다.(name속성값=value값)
		 * 
		 * request의 parameter 영역으로부터 데이터를 뽑는 메서드
		 * - request.getParameter("키값") : String(그에 해당하는 value값) -> 무조건 문자열로 반환됨.
		 * - request.getParameterValues("키값") : String[](그에 해당하는 value값)
		 * => 하나의 key값으로 여러 개의 value값을 받는 경우(체크박스) 문자열 배열형으로 반환 가능
		 */
		...
		//체크박스처럼 복수개의 정보를 받을때는 배열형태로 받아야함.
				...
				if(foods == null) {
					...
					//배열에 있는 모든 값들을 구분자를 통해서 하나의 문자열로 반환해 주는 메서드
				}
				
				// 뽑아낸 값들을 가지고 요청 처리해야 함 (DB와 상호작용)
				// 일반적인 흐름 : Servlet -> Service(뽑아낸 값들을 넘기면서 비지니스로직 실행) -> Dao 호출 - DB SQL문 실행 - 결과값 반환
				
				// 위의 과정을 다 처리한 후 사용자가 보게 될 응답 페이지를 만들어서 전달해 주기
				
				/*
				 * 장점 : Java 코드 내에 작성하기 때문에 반복문, 조건문, 유용한 메서드를 사용할 수 있다.
				 * 
				 * 단점 : 복잡, 혹시라도 나중에 html 수정해야 할 때 Java 코드 내에서 수정이 이루어지기 때문에
				 *       수정된 내용을 다시 반영시키고자 한다면 서버를 재실시 해야 한다. 사소한 오타로 에러 날 수 있다.
				 */
				
				//* response 객체를 통해 사용자에게 html(응답 화면)을 전달
				// 1) 이제부터 내가 출력할 내용은 문서 형태이고 html이다. 더불어서 문자셋은 utf-8이라는 것을 지정
